...
class PosSession(models.Model):
    ...
    def _loader_params_pos_payment_method(self):
        ...
        result['search_params']['fields'].append('equipment_id')
        return result
...
class PosPaymentMethod(models.Model):
    ...
    def execute_request_receipt_create_model(self, order_reference):
        pos_order_id = self.env['pos.order'].search(
            [('pos_reference', '=', order_reference[0] if isinstance(order_reference, list) else order_reference)])
        if pos_order_id:
...
class PaxPayment(models.Model):
    ...
    pax_payment_description = fields.Char(string='Mensaje',
                                          help='Mensaje de texto que representa la respuesta de la operación puede ser Aprobado o Rechazado.')
